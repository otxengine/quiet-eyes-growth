...
3. **Render מגיש קבצים לפני rewrite** ("Render does not apply rewrite rules to a path if a resource exists at that path") — לכן כל URL שיווקי מקבל את הקובץ שלו, וראוטים של האפליקציה נופלים ל-`index.html`. **בכוונה אין `dist/404.html`** — הוא היה גובר על ה-rewrite ושובר רענון בעמודי אפליקציה.
