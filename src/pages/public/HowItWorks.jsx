...
const steps = [
  {
    ...
    mockup: (
      <div className="rounded-2xl border border-[#eee] bg-white p-5 max-w-[380px]" style={{ boxShadow: '0 20px 60px rgba(0,0,0,0.06)' }}>
        <span className="text-[11px] font-semibold text-[#222] block mb-3">הרשם ל-Cortexi</span>
        <div className="space-y-2.5">
          {[{ label: 'שם העסק', value: 'מסעדת הגינה' }, { label: 'קטגוריה', value: 'מסעדה' }, { label: 'עיר', value: 'תל אביב' }].map(f => (
            <div key={f.label}>
              ...
              <div className="rounded-lg border border-[#e8e8e8] bg-gray-50 px-3 py-2 text-[10px] text-[#444]">{f.value}</div>
            </div>
          ))}
          <div className="rounded-lg text-white text-center py-2.5 text-[10px] font-semibold mt-2" style={{ background: G }}>התחל סריקה ←</div>
        </div>
      </div>
    ),
  },
  {
    ...
    mockup: (
      <div className="rounded-2xl border border-[#eee] bg-white p-5 max-w-[380px] text-center" style={{ boxShadow: '0 20px 60px rgba(0,0,0,0.06)' }}>
        <div className="w-16 h-16 rounded-full border-4 border-[#E8344D]/20 border-t-[#E8344D] mx-auto mb-3 animate-spin" style={{ animationDuration: '1.5s' }} />
        ...
        <div className="space-y-1 text-[9px] text-[#999]">
          ...
          <p className="text-[#E8344D] animate-pulse">⟳ מנתח מתחרים...</p>
      ...
      <div className="rounded-2xl border border-[#eee] bg-white p-4 max-w-[380px]" style={{ boxShadow: '0 20px 60px rgba(0,0,0,0.06)' }}>
        ...
        <div className="space-y-2">
          {[
            ...
            <div key={i} className="rounded-lg bg-gray-50 p-2.5 flex items-start gap-2" style={{ borderRight: `2px solid ${s.color}` }}>
  ...
  {
    number: '4',
    title: 'דשבורד מלא — עכשיו אתה בשליטה ב-Cortexi',
    ...
    mockup: (
      <div className="rounded-2xl border border-[#eee] bg-white overflow-hidden max-w-[380px]" style={{ boxShadow: '0 20px 60px rgba(0,0,0,0.06)' }}>
        <div className="p-3 border-b border-gray-100 bg-gray-50" style={{ borderRight: '3px solid #10b981' }}>
        ...
        <div className="grid grid-cols-4 gap-1 p-2">
          {[{ v: '12', l: 'תובנות' }, { v: '8', l: 'מתחרים' }, { v: '47', l: 'ביקורות' }, { v: '23', l: 'לידים' }].map(c => (
            <div key={c.l} className="rounded-md border border-gray-100 p-1.5 text-center">
  ...
  return (
    <div className="min-h-screen" style={{ background: '#0A0A0F' }}>
      <section className="px-6 pt-20 pb-12 max-w-5xl mx-auto text-center">
        <h1 className="text-[32px] md:text-[40px] font-bold text-white mb-4">מהרשמה לתובנה ראשונה — 60 שניות</h1>
        <p className="text-[15px]" style={{ color: '#9090A8' }}>ארבעה צעדים פשוטים. בלי הגדרות מסובכות.</p>
      </section>
            ...
            <div key={i} className={`grid lg:grid-cols-2 gap-12 items-center ${i % 2 === 1 ? 'direction-ltr' : ''}`}>
              <div className={i % 2 === 1 ? 'lg:order-2' : ''}>
                <div className="flex items-center gap-3 mb-3">
                  <span
                    className="w-8 h-8 rounded-full text-white text-[14px] font-bold flex items-center justify-center"
                    style={{ background: G }}
                  >
                    {step.number}
                  </span>
                ...
                <p className="text-[14px] leading-relaxed" style={{ color: '#9090A8' }}>{step.desc}</p>
              </div>
      ...
      <section className="px-6 py-20" style={{ background: G }}>
        <div className="max-w-3xl mx-auto text-center">
          <h2 className="text-[28px] font-bold text-white mb-4">מוכן להתחיל?</h2>
          <p className="text-[14px] text-white/80 mb-8">הרשמה חינם. בלי כרטיס אשראי. תובנה ראשונה תוך 60 שניות.</p>
          <button
