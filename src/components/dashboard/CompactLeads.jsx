...
function statusLabel(status) {
  ...
  return { text: 'קר', cls: 'text-foreground-muted' };
}
      ...
      <div className="divide-y divide-[#f5f5f5]">
        {leads.slice(0, 3).map((lead) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-[11px] font-medium text-foreground-secondary flex-1 truncate">{lead.name}</span>
        ...
        {leads.length === 0 && (
          <div className="p-6 text-center flex flex-col items-center justify-center">
            <CheckCircle className="w-8 h-8 text-foreground-muted/50 mb-1.5" />
            <p className="text-[11px] text-foreground-muted">עוד אין לידים — המערכת מחפשת בשבילך</p>
          </div>
        )}
      </div>
      {contactLead && (
