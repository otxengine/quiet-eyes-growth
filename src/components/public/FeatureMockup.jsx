    ...
    <MockupFrame>
      <div className="p-4 border-b border-border/50">
        <div className="flex gap-2 mb-3">
          {['הכל', 'איומים', 'הזדמנויות', 'מגמות'].map((t, i) => (
            <span key={t} className={`px-3 py-1 rounded-md text-[9px] font-medium ${i === 0 ? 'bg-[#111] text-white' : 'bg-secondary/60 text-[#999]'}`}>{t}</span>
          ))}
        </div>
      </div>
      <div className="p-3 space-y-2">
        {[
          ...
          <div key={i} className="rounded-lg p-3 bg-secondary/50 hover:bg-secondary/60 transition-colors" style={{ borderRight: `2.5px solid ${s.color}` }}>
    ...
    <MockupFrame>
      ...
      <div className="p-3 space-y-2">
        {[
          ...
          <div key={i} className="rounded-lg border border-border/50 p-3" style={{ borderRight: `2px solid ${r.sentColor}` }}>
    ...
    <MockupFrame>
      <div className="p-4 border-b border-border/50">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-1.5">
            ...
            <span className="px-2 py-0.5 rounded text-[8px] bg-secondary/60 text-[#999]">Pipeline</span>
          </div>
        </div>
      </div>
      <div className="p-3 space-y-2">
      ...
      <div className="px-3 pb-3">
        <div className="flex rounded-lg overflow-hidden text-[7px] font-medium">
          <div className="bg-secondary text-foreground-secondary px-2 py-1 flex-1 text-center">3 חדש</div>
      ...
      <div className="p-3 space-y-2">
        ...
        <div className="rounded-lg border border-border/50 p-3">
          ...
          <div className="flex gap-1">
            {[1,2,3,4,5].map(n => (
              <span key={n} className={`w-6 h-6 rounded flex items-center justify-center text-[10px] ${n === 4 ? 'bg-[#d97706] text-white' : 'bg-secondary/60 text-[#999]'}`}>{n}</span>
            ))}
          </div>
        </div>
      </div>
