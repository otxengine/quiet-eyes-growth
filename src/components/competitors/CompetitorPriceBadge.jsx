    ...
    <div className="space-y-1.5">
      {competitor.last_known_prices && (
        <div className="flex items-start gap-1.5">
          <Banknote className="w-3.5 h-3.5 text-foreground-muted/50 mt-0.5" />
          <div>
            <span className="text-[10px] font-medium text-foreground-muted block mb-0.5">מחירים ידועים</span>
            <p className="text-[11px] text-foreground-secondary">{competitor.last_known_prices}</p>
          </div>
        </div>
      )}
      {competitor.last_price_check && (
        <span className="text-[9px] text-foreground-muted/50 block">עדכון אחרון: {timeAgo(competitor.last_price_check)}</span>
      )}
      {recentChange && (
