    ...
    <div className="bg-white rounded-[10px] border border-border/50 hover:border-border transition-colors">
      <div className="p-3 flex items-center gap-3 cursor-pointer" onClick={() => setExpanded(!expanded)}>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            {prediction.timeframe && <span className="text-[9px] text-foreground-muted/70">⏱ {prediction.timeframe}</span>}
          </div>
        ...
        <div className="flex items-center gap-2">
          <div className="text-center">
            ...
            <span className="text-[8px] text-foreground-muted/50 block">ביטחון</span>
          </div>
      ...
      {expanded && (
        <div className="px-4 pb-4 pt-1 border-t border-border/40 space-y-3">
          <p className="text-[12px] text-foreground-secondary leading-relaxed">{prediction.summary}</p>
            ...
            <div>
              ...
              <div className="space-y-1">
                {actions.map((action, i) => (
                  <div key={i} className="flex items-start gap-1.5 text-[11px] text-foreground-secondary">
          ...
          {prediction.data_sources && (
            <p className="text-[9px] text-foreground-muted/70">מבוסס על: {prediction.data_sources}</p>
          )}
          ...
          {prediction.status === 'active' && (
            <div className="flex gap-2 pt-1">
              ...
              <button onClick={(e) => { e.stopPropagation(); updateMutation.mutate('dismissed'); }}
                className="flex items-center gap-1 px-3 py-1.5 rounded-md text-[11px] font-medium text-foreground-muted bg-white border border-border/60 hover:border-border-hover transition-colors">
