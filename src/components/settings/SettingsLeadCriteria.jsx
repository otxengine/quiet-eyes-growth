...
export default function SettingsLeadCriteria({ form, setForm, onSave }) {
  ...
  return (
    <div className="bg-white rounded-[10px] border border-border/50 p-5 space-y-4">
      ...
      <p className="text-[12px] text-foreground-muted">הגדר מה נחשב ליד איכותי עבורך — הנתונים מועברים לסוכני ה-AI</p>
      <div>
        <label className="text-[12px] text-foreground-muted mb-1 block">תקציב מינימום</label>
        <div className="flex flex-wrap gap-1.5">
          {budgetOptions.map((b) => (
            <button key={b} onClick={() => setForm({ ...form, min_budget: b })}
              className={`px-2.5 py-1 rounded-md text-[11px] font-medium transition-colors ${form.min_budget === b ? 'bg-[#111111] text-white' : 'text-foreground-muted/70 border border-border/60 hover:border-border-hover'}`}>{b}</button>
          ))}
        </div>
      </div>
      <div>
        <label className="text-[12px] text-foreground-muted mb-1 block">סוגי שירות רלוונטיים</label>
      ...
      <div>
        <label className="text-[12px] text-foreground-muted mb-1 block">אזור גיאוגרפי מועדף</label>
      ...
      <div>
        <label className="text-[12px] text-foreground-muted mb-1 block">סימני כוונת קנייה</label>
        <textarea
        ...
        />
        <p className="text-[10px] text-foreground-muted/50 mt-1">ביטויים שמצביעים על כוונת רכישה — הסוכן ישתמש בהם לסינון לידים איכותיים</p>
      </div>
      <div>
        <label className="text-[12px] text-foreground-muted mb-1 block">הגדרת ליד איכותי (בשפה חופשית)</label>
        <textarea
          ...
          className={textareaCls}
        />
        <p className="text-[10px] text-foreground-muted/50 mt-1">הסבר לסוכן ה-AI מה הופך ליד לאיכותי עבורך</p>
      </div>
      ...
      <p className="text-[11px] text-foreground-muted/50">הקריטריונים מועברים לסוכני ה-AI ומשפיעים על ניקוד הלידים האוטומטי</p>
    </div>
  );
}
