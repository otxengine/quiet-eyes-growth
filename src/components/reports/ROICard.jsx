...
function StatBox({ label, value, sub, color = 'indigo' }) {
  const colors = {
    ...
    gray:   'bg-secondary/50   border-border/60   text-foreground-secondary',
  };
  return (
...
function MonthBar({ month, total, system, max }) {
  ...
  return (
    <div className="flex items-end gap-1 flex-col w-full">
      ...
      <span className="text-[9px] text-foreground-muted/70 text-center w-full">{month}</span>
    </div>
  );
}
  ...
  if (isLoading) {
    return (
      <div className="rounded-2xl border border-border/60 bg-white px-5 py-8 shadow-sm flex items-center justify-center gap-3">
        ...
        <span className="text-[12px] text-foreground-muted/70">מחשב ROI...</span>
      </div>
    );
  }
  ...
  if (!data?.summary) {
    return (
      <div className="rounded-2xl border border-border/60 bg-white px-5 py-6 shadow-sm text-center">
        <DollarSign className="w-8 h-8 text-border mx-auto mb-2" />
        <p className="text-[12px] text-foreground-muted/70">אין עדיין נתוני ROI</p>
        <p className="text-[10px] text-foreground-muted/50 mt-1">ה-ROI מחושב לאחר סגירת עסקאות ראשונות</p>
      </div>
    );
  }
    ...
    <div className="rounded-2xl border border-border/60 bg-white shadow-sm overflow-hidden">
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-border/40">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[13px] font-bold text-foreground">ROI — תשואה על השקעה</span>
        </div>
        <button
          onClick={() => refetch()}
          ...
          title="רענן"
        >
          <RefreshCw className={`w-3.5 h-3.5 text-foreground-muted/70 ${isRefetching ? 'animate-spin' : ''}`} />
        </button>
      </div>
      ...
      <div className="px-5 py-4 space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div>
            <p className="text-[10px] text-foreground-muted/70 mb-1">ROI מוערך</p>
            <p className={`text-[32px] font-black ${roiColor}`}>{summary.roi_percent}%</p>
            {summary.payback_days && (
              <p className="text-[10px] text-foreground-muted/70">החזר השקעה תוך ~{summary.payback_days} ימים</p>
            )}
          </div>
          <div className="text-left">
            <p className="text-[10px] text-foreground-muted/70 mb-1">השפעת הכנסה</p>
            <p className="text-[20px] font-bold text-foreground">₪{summary.total_system_impact.toLocaleString()}</p>
            <p className="text-[10px] text-foreground-muted/70">עלות חודשית: ₪{summary.subscription_cost_monthly}</p>
          </div>
        </div>
        ...
        {monthly_breakdown.length > 0 && (
          <div>
            <p className="text-[10px] text-foreground-muted/70 mb-2">פילוח חודשי (6 חודשים אחרונים)</p>
            ...
            <div className="flex items-center gap-3 mt-2">
              <span className="flex items-center gap-1 text-[9px] text-foreground-muted/70"><span className="w-2 h-2 rounded-sm bg-primary/15 inline-block" /> סה"כ הכנסה</span>
              <span className="flex items-center gap-1 text-[9px] text-foreground-muted/70"><span className="w-2 h-2 rounded-sm bg-primary inline-block" /> מיוחס מערכת</span>
            </div>
          </div>
        )}
