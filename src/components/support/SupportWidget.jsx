...
export default function SupportWidget({ businessProfile }) {
  ...
  const [hasOpenTicket, setHasOpenTicket] = useState(() => {
    try { return localStorage.getItem('cortexi_support_ticket_open') === '1'; } catch { return false; }
  });

  useEffect(() => {
    const handler = () => {
      try { setHasOpenTicket(localStorage.getItem('cortexi_support_ticket_open') === '1'); } catch {}
    };
  ...
  const handleTicketCreated = () => {
    try { localStorage.setItem('cortexi_support_ticket_open', '1'); } catch {}
