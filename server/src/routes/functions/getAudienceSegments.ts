...
export interface AudienceExtras {
  insight_text?: string;
  action_type?: string;
  post_content?: string;
...
/**
 * Loads the same real-business intelligence (reviews/leads/signals/
 * competitors/trends) getAudienceSegments uses, and builds the Hebrew
 * context block fed to the LLM. Shared with discussTargetAudience.ts so
 * both the one-shot generator and the conversational flow ground their
 * answers in the same real data.
 */
export async function buildAudienceIntelligence(businessProfileId: string, extras: AudienceExtras = {}) {
  const { insight_text, action_type, post_content, image_description, platform, objective } = extras;
...
`.trim();
  ...
  return { profile, fullContext, dataQuality, dataSources };
}
...
export async function getAudienceSegments(req: Request, res: Response) {
  ...
  try {
    const intel = await buildAudienceIntelligence(businessProfileId, {
      insight_text, action_type, post_content, image_description, platform, objective,
    });
    if (!intel) return res.status(404).json({ error: 'Business profile not found' });
    const { profile, fullContext, dataQuality, dataSources } = intel;
