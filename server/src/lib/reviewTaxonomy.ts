...
import type { Aspect } from './reviewTopicPacks';
...
// Fallback set used when no sector topic set is provided
const CORE_TOPIC_SET = new Set<string>(TOPIC_IDS);
const CORE_ALLOWED   = TOPIC_IDS.join(', ');

/**
 * KAN-123 — shared aspect extractor for own + competitor reviews.
 * KAN-142 — pass `topicSet` (from resolveTopicSet) to inject the full sector pack into the prompt.
 * Returns up to 6 topic IDs with wording-derived polarity, ranked by salience when >6 qualify.
...
export async function batchExtractTopics(
  ...
  const allowedIds  = topicSet ? topicSet.map(a => a.id) : [...TOPIC_IDS];
  const allowedSet  = topicSet ? new Set(allowedIds) : CORE_TOPIC_SET;
  const allowedStr  = topicSet
    ? topicSet.map(a => `${a.id} (${a.label_he})`).join(', ')
    : CORE_ALLOWED;
  ...
  try {
    ...
    return reviews.map((_, i) => {
      ...
      const valid = (item.topics as string[]).filter(t => allowedSet.has(t)).slice(0, 6);
