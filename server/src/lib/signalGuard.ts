...
/**
 * source_origin values that represent an actual Google review (as opposed to
 * Tavily/manual/etc.). Single shared list — previously duplicated across
 * computeThemeRollup/getCompetitorReviewInsights/reputationAdvisorChat/topicTimeline,
 * which is how dataforseo_google_reviews silently fell out of sync with the others.
 */
export const GOOGLE_REVIEW_SOURCES = ['google_business_api', 'google_places', 'serp_google_maps_reviews', 'dataforseo_google_reviews'];

const REVIEW_ORIGINS = new Set([...GOOGLE_REVIEW_SOURCES, 'tavily', 'apify', 'manual']);
